  ...
  <v-card class="citation-tile fill-height" variant="flat" rounded="lg">
    <div class="tile-inner d-flex flex-column">
      ...
      <div v-else class="citation-list flex-fill">
        ...
        >
          <div class="citation-meta d-flex align-center">
            <v-chip size="x-small" variant="tonal" :color="scoreColor(item.score)">
              {{ (item.score * 100).toFixed(0) }}%
            </v-chip>
            <span class="citation-source text-caption font-weight-medium text-truncate">
              {{ item.source }}
            </span>
            <span v-if="item.page" class="citation-page text-caption text-grey-darken-1 ml-auto">
              第{{ item.page }}页
            </span>
          </div>
