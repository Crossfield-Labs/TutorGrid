...
from workers.models import WorkerArtifact, WorkerControlRef, WorkerProgressEvent, WorkerResult, WorkerSessionRef
...
ControlCallback = Callable[[WorkerControlRef | None], Awaitable[None]]
...
class ClaudeSdkWorker(WorkerAdapter):
    ...
    def description(self) -> str:
        return "Delegate broader agentic coding, documentation, research, or learning work to the local Claude Agent SDK."
    ...
    def _build_can_use_tool_handler(
        ...
        *,
        ...
        async def can_use_tool(tool: str, tool_input: dict[str, Any], _context: ToolPermissionContext) -> Any:
            ...
            if (
                normalized_tool == "bash"
                and bool(tool_input.get("dangerouslyDisableSandbox"))
                and normalized_tool not in explicit_allowed
            ):
    ...
    def _select_profile(self, requested_profile: str | None) -> str:
        ...
        if normalized not in {"code", "doc", "study", "research"}:
            return "code"
        return normalized
    ...
    def _profile_rules(profile: str) -> dict[str, list[str]]:
        if profile == "doc":
            return {
                "allowed": ["Read", "Write", "Edit", "LS", "Glob", "Grep"],
                "disallowed": ["Bash"],
            }
        if profile == "study":
            return {
        ...
        return {
            "allowed": ["Read", "Write", "Edit", "LS", "Glob", "Grep", "Bash"],
            "disallowed": [],
        }
    ...
    def _summarize_server_info(server_info: Any) -> str:
        ...
        if not provider and not model_count and not agent_count:
            return ""
        provider_text = f"provider={provider}" if provider else "provider=unknown"
        return f"Claude server info ready ({provider_text}, models={model_count}, bundled agents={agent_count})."
